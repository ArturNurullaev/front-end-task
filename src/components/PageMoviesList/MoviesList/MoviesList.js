...
componentWillReceiveProps (nextProps){	this.getList(nextProps.page)	}
...
delete movie.genre_ids;
  ...
  render() {
	 ...
	 let moviesList = this.state.data ? this.state.data.results.map((item)=> {
		return <MovieItem title={item.title.toUpperCase()} key={item.id} movieID={item.id} image={item.poster_path} popularity={item.popularity} releaseDate={item.release_date} votes={item.vote_average} genres={item.genres} onClick={this.props.onClick}/>
	}) : <div></div>;
